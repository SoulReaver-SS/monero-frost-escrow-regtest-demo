...
export async function checkToolInvocationValidity(invo) {
    ...
    if (invo.tool.tool_id == "001") {
        ...
        const checkUrl = `${invo_link.origin}/monerochan001/${invo.tool.payload.address}`;
        try {
...
export async function shareViewKey002(payload) {
...
// client wallet side
export async function potentialSuccessRedirect002(payload) {
...
// backend response
export async function handle002ShareRequest(req, wallets, parsed_cb, successUrl) {
    try {
        const json_body = await req.json();
        const { viewkey, primary_address, wallet_slot } = json_body;
        ...
        const foundSlot = wallets.find((wallet) => wallet.wallet_slot === wallet_slot);
        if (foundSlot) {
