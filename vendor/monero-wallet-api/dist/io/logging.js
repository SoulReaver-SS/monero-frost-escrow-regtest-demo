...
export async function setupLoggingPath(scan_settings_path, path_prefix, role, cpu_worker_id) {
    ...
    globalThis.__global_log_setup = global_log_setup;
        ...
        return JSON.stringify(value, (_key, v) => {
            ...
            if (v !== null && typeof v === "object") {
                ...
                if (typeof v.then === "function") {
                    try {
                        return typeof Bun !== "undefined" && Bun.inspect
                            ? Bun.inspect(v)
                            : "Promise";
                    }
                    catch {
...
const flusherRef = new WeakMap();
function startLogFlusher(setup) {
    if (setup.logs !== "file" && setup.logs !== "console-and-file")
        return;
    if (flusherRef.has(setup))
        return; // already running
    let writtenIndex = 0;
    const id = setInterval(async () => {
        const buffer = setup.file_logbuffer;
        if (buffer.length <= writtenIndex)
            return; // nothing new
...
export function log(fnname, message) {
    const setup = globalThis.__global_log_setup;
    if (!setup)
        return;
    ...
    if (logs_include && logs_include.length > 0 && !logs_include.includes(fnname))
        return;
    if (logs_exclude && logs_exclude.includes(fnname))
        return;
    ...
    if (logPath) {
        const msg = message;
        if ((typeof msg === "string" && msg.includes(logPath)) ||
            (Array.isArray(msg) &&
    ...
    if (logs === "console" || logs === "console-and-file") {
        console.log(`[${fnname}]`, ...(Array.isArray(message) ? message : [message]));
    }
