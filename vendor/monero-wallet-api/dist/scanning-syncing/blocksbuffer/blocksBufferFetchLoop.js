import { findRange, get_block_headers_range, get_info, mergeRanges, NodeUrl, sleep, } from "../../api";
import {} from "../connectionStatus";
...
export async function* blocksBufferFetchLoop(node_url, start_height, blocks_buffer, // pass by reference
connection_status, // we make a local copy of this and pass last_packet and sync updates seperately
max_blocks_buffer_size = MAX_BLOCKS_BUFFER_SIZE, anchor_range, stopSync) {
    ...
    let { current_range, scanned_ranges } = await initScannedRanges(nodeUrl.node_url, start_height, anchor_range ? [anchor_range] : []);
    ...
    while (true) {
        ...
        if (blocks_buffer.length >= max_blocks_buffer_size) {
            connection_status.last_packet = {
                ...
                node_url: node_url,
                timestamp: new Date().toISOString(),
                daemon_height: connection_status.sync.daemon_height ||
        ...
        log("blocksBufferFetchLoop", "response: " + result_meta.block_infos.length + " blocks");
        ...
        try {
            const parse_result = await updateBlocksBufferScanHeight(current_range, result_meta, connection_status.sync.scanned_ranges);
            ...
            if (parse_result.split_height) {
                ...
                if (!connection_status.sync.reorg_info) {
                    ...
                    connection_status.sync.reorg_info.split_heights.push(parse_result.split_height);
                }
...
export function makeBlocksBufferItem(result_meta, get_blocks_bin) {
    const bufferItem = {
...
export function popBlocksBufferItemFromIndex(blocks_buffer, index) {
    if (index < 0 || index >= blocks_buffer.length)
        return;
    return blocks_buffer.splice(index, 1)[0];
}
export function findBufferitemBySplitHeight(blocks_buffer, split_height) {
    for (const item of blocks_buffer) {
        ...
        if (start <= split_height.block_height &&
            split_height.block_height <= end) {
            return item;
        }
    }
}
export function findBufferItemIndexByLocalId(blocks_buffer, local_uuid) {
    return blocks_buffer.findIndex((item) => item.local_uuid === local_uuid);
}
export function popBlocksBufferItemsFromSplitHeight(blocks_buffer, split_height) {
    ...
    const foundIndex = findBufferItemIndexByLocalId(blocks_buffer, found.local_uuid);
    if (foundIndex < 0) {
...
export async function doRPCrequest(nodeUrl, current_range, stopSync) {
...
export async function initScannedRanges(node_url, start_height, scanned_ranges = []) {
    {
...
export async function updateBlocksBufferScanHeight(current_range, result_meta, scanned_ranges) {
    ...
    if (!last_block_hash_of_result)
        return { current_range, scanned_ranges }; // block_infos empty, no change (we are at tip and there was no new block)
    ...
    if (!first_block_hash)
        return { current_range, scanned_ranges }; // block_infos empty, no change (we are at tip and there was no new block) current_range; // should never happen, if there is last_block_hash there should be first_block_hash
    ...
    if (!(first_block_hash.block_hash === current_blockhash.block_hash)) {
        return await handleBlocksBufferReorg(current_range, result_meta, scanned_ranges, oldRange);
    }
...
export function makeNewBlocksBufferScanRange(newRange, scanned_ranges) {
    scanned_ranges.push(newRange);
    // merge existing ranges & find end of current range
...
export async function handleBlocksBufferReorg(current_range, result_meta, scanned_ranges, oldRange) {
    ...
    for (const block_hash of oldRange.block_hashes) {
        const split_height_index = result_meta.block_infos.findIndex((b) => b.block_hash === block_hash.block_hash);
        ...
        // fix current_range
        let anchor = result_meta.block_infos.slice(-100)[0]; // if we got lots of new blocks
        const old_anchor = oldRange.block_hashes.at(-1);
        if (!anchor &&
            old_anchor &&
    ...
    throw new CatastrophicReorgError("Could not find reorg split height. Most likely connected to faulty node / catastrophic reorg.");
}
/**
 ...
 */
export async function reduceStartHeightToTip(start_height, node_url) {
