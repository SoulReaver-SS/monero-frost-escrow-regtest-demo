...
import { openScanSettingsFile, SCAN_SETTINGS_STORE_NAME_DEFAULT, sleep, } from "../api";
...
export async function createWebworker(handle_result, scan_settings_path, pathPrefix, handle_error) {
    try {
        ...
        const cpu_worker_count = typeof scanSettings?.cpu_worker_count !== "undefined"
        ...
        for (let i = 0; i < cpu_worker_count; i++) {
            ...
            cpuWorker.onerror = (e) => {
                if (handle_error)
                    handle_error("cpu worker " + i + " error: " + (e.message ?? String(e)));
            };
        ...
        coordinationWorker.postMessage({
            ...
            pathPrefix,
            role: "coordinator",
        }, cpuPorts); // transfer CPU ports to coordination worker
        log("createWebworker", [
        ...
        const waitHeap = (w, path, timeoutMs = 15000) => new Promise((resolve, reject) => {
            const t = setTimeout(() => {
...
export function startWebworkerReady() {
