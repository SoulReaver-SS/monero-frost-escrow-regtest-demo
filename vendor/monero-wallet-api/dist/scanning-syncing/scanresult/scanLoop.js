...
export function makeWorkItem(walletConfig, batch, from, to) {
...
export async function* scanLoop(wallet) {
    let scanResult;
    let work_uuid;
    let workitem_batch_uuid;
    let loaded_batch_uuid;
    while (true) {
        const item = yield {
            type: "Ready",
            work_uuid,
        ...
        for (let i = item.from; i <= item.to; i++) {
            ...
            if (typeof input === "object")
                throw new Error("cancel oldworkitem, before sending new workitem, while work in progress");
