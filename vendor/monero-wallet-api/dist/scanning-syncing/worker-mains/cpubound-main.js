...
export async function handleCpuboundScan(msg, cpu_worker_status) {
    ...
    log("handleCpuboundScan", "got scan msg, walletConfig=" +
        msg.walletConfig?.primary_address?.slice(0, 6));
    const walletConfig = msg.walletConfig;
    const item = msg;
    log("handleCpuboundScan", "primed, feeding workItem uuid=" +
        (item && typeof item === "object"
            ? item.work_uuid?.slice(0, 8)
            : "cancel"));
    // feed work item, get first InProgress yield
    let blockCount = 0;
    const heighfrom = item.batch.get_blocks_result_meta.block_infos[item.from].block_height;
    const heightto = item.batch.get_blocks_result_meta.block_infos[item.to].block_height;
    const viewpair = await ViewPair.create(item.walletConfig.primary_address, item.walletConfig.secret_view_key, item.walletConfig.subaddress_index);
    ...
    if (!first_block_meta)
        throw new Error("no first block meta");
    const scanResult = {
        outputs: [],
        all_key_images: [],
...
export function handleCpuboundScanTry(msg, cpu_worker_status) {
    try {
...
export function sendFromCpuWorker(port, msg) {
    port.postMessage(msg);
}
export function sendToCpuWorker(port, msg) {
    port.postMessage(msg);
}
