 ...
 */
export async function writeScanSettings(scan_settings, scan_settings_path = SCAN_SETTINGS_STORE_NAME_DEFAULT) {
    return await atomicWrite(scan_settings_path, JSON.stringify(scan_settings, null, 2));
}
export async function writeScanSettingsFileDefaultLocation(params) {
...
export async function writeStartHeightToScanSettings(start_height, scan_settings_path = SCAN_SETTINGS_STORE_NAME_DEFAULT) {
...
export async function writeDaemonHeightAsStartHeightToScanSettings(node_url, scan_settings_path = SCAN_SETTINGS_STORE_NAME_DEFAULT) {
...
export async function cullTooLargeScanHeight(node_url, scan_settings_path = SCAN_SETTINGS_STORE_NAME_DEFAULT) {
    const getInfo = await get_info(node_url);
    const scanSettings = (await openScanSettingsFile(scan_settings_path)) || {
        node_url: LOCAL_NODE_DEFAULT_URL,
        wallets: [],
...
export async function writeNodeUrlToScanSettings(node_url, scan_settings_path = SCAN_SETTINGS_STORE_NAME_DEFAULT) {
    const scanSettings = (await openScanSettingsFile(scan_settings_path)) || {
        node_url: LOCAL_NODE_DEFAULT_URL,
        wallets: [],
        start_height: null,
    ...
    return await atomicWrite(scan_settings_path, JSON.stringify(scanSettings, null, 2));
}
export async function readNodeUrlFromScanSettings(scan_settings_path = SCAN_SETTINGS_STORE_NAME_DEFAULT) {
    const scanSettings = await openScanSettingsFile(scan_settings_path);
 ...
 * @returns A promise that resolves when the file is successfully written.
 */
export async function writeMerchantConfirmationsToScanSettings(merchant_confirmations, scan_settings_path = SCAN_SETTINGS_STORE_NAME_DEFAULT) {
    const scanSettings = (await openScanSettingsFile(scan_settings_path)) || {
        node_url: LOCAL_NODE_DEFAULT_URL,
        wallets: [],
        start_height: null,
 ...
 */
export async function readMerchantConfirmationsFromScanSettings(scan_settings_path = SCAN_SETTINGS_STORE_NAME_DEFAULT) {
    const scanSettings = await openScanSettingsFile(scan_settings_path);
...
export async function writeWalletToScanSettings(params) {
            ...
            wallets: [
                {
                    ...
                    subaddress_index: params.subaddress_index || SUB_ADDRESS_INDEX_DEFAULT_VALUE,
    ...
    if (params.logs)
        scanSettings.logs = params.logs;
    if (params.logs_include)
        scanSettings.logs_include = params.logs_include;
    if (params.logs_exclude)
        scanSettings.logs_exclude = params.logs_exclude;
...
export function getNonHaltedWallets(scan_settings) {
    if (!scan_settings)
        return [];
 ...
 */
export function getPathPrefix(scan_settings_path = SCAN_SETTINGS_STORE_NAME_DEFAULT, pathPrefix) {
